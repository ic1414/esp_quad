...
void MPU::update(){
  ...
  if(millis() - timer_sample1 >= 5){
    ...
    angle[X] +=  cur_Gyro[X] * dta * gyroAngleWeight;
    angle[Y] +=  cur_Gyro[Y] * dta * gyroAngleWeight;
    angle[Z] +=  cur_Gyro[Z] * dta * gyroAngleWeight;
    angle[X] += angle[Y] * sin(cur_Gyro[Z] * dta * 0.017453292f) * gyroAngleWeight;
    angle[Y] -= angle[X] * sin(cur_Gyro[Z] * dta * 0.017453292f) * gyroAngleWeight;
    timer_angle = micros();
  ...
  if(millis() - timer_sample3 >= 1){
    ...
    if(cur_Acc[Y] < total_Acc)cur_AccAngle[X] = -(asin(cur_Acc[Y]/total_Acc)* 57.296);
    if(cur_Acc[X] < total_Acc)cur_AccAngle[Y] = -(asin(cur_Acc[X]/total_Acc)* 57.296);
