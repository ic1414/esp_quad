...
SimpleKalmanFilter filterX(0.2f, 0.2f, 0.02f);
SimpleKalmanFilter filterY(0.2f, 0.2f, 0.02f);
SimpleKalmanFilter filterZ(0.2f, 0.5f, 0.02f);
//method 2
...
void loop() {
  ...
  while (fifoCount < packetSize) {
    count += 1;
    count2 += 1; // "count2" to get fifo for every 10 iterations
    getAnglesV2(); // update angle from complementary filter
    mpu_update_flag_core1 += 1; // you guess
    ...
    if (millis() - timer > 10)
    {
      ...
      timer = millis();
      count = 0; // reset sample
    }

  }

  // get fifo angle val and fuse with c filter val
  if ((fifoCount == 1024) or (fifoCount % packetSize != 0)) {
    ...
    angleZ = yrp[0] * 180 / PI;
    count += 1; // update sample
  }

}


// gyro rate is 8kHZ, acc rate is 1kHZ
// get acc less frequently to increase overall rate
// higher rate
void getAnglesV2() {
  ...
  // acc
  // get acc raw data
  // if have 8 gyro samples, get one acc to fix gyro drifting
  if (count_gyro == 8) {
    // get raw acc
    Wire.beginTransmission(0x68);
    Wire.write(0x3B);
