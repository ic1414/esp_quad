...
SimpleKalmanFilter filterA(50, 50, 1);
SimpleKalmanFilter filterB(50, 50, 1);
SimpleKalmanFilter filterC(50, 50, 1);
SimpleKalmanFilter filterD(50, 50, 1);
...
void task_main( void * pvParameters ) {
  ...
  for (;;) {
    ...
    // 注意!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //建议用角速度环控制z轴，具体可以打开串口监视器看电机转到大概+180 或 -180的输出 
    ...
    float cur_AngleZ = imu.cur_Angle[2]; //因为dmp输出z角度范围在-180-180
    if (zRadio == true) {
      ...
      // 判断最小路径
      if(abs(setPointZ - cur_AngleZ) <= 180.0f){
